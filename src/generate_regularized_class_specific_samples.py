...
class RegularizedClassSpecificImageGeneration():
    ...
    def __init__(self, model, target_class, save_path):
    ...
    def generate(self, iterations=1000, blur_freq=4, blur_rad=1, wd=0.0001, clipping_value=0.1):
        ...
        for i in range(1, iterations):
            ...
            if i in np.linspace(0, iterations, 10, dtype=int):
                # Save image
                im_path = self.save_path + f'/generated/class_{self.target_class}/c_{self.target_class}_iter_{i}_loss_{class_loss.data.cpu().numpy()}.jpg'
        ...
        with open(self.save_path + f'/generated/class_{self.target_class}/run_details.txt', 'w') as f:
        ...
        os.rename(self.save_path + f'/generated/class_{self.target_class}',
                  self.save_path + f'/generated/class_{self.target_class}_blurfreq_{blur_freq}_blurrad_{blur_rad}_wd{wd}')
